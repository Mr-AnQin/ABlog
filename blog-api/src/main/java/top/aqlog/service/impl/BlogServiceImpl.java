...
import top.aqlog.mapper.BlogMapper;
...
import top.aqlog.util.markdown.MarkdownUtils;
...
public class BlogServiceImpl implements BlogService {
     ...
     */
    //@Retryable(MethodTheRetryMechanismIsException.class)
    private void setBlogViewsFromRedisToPageResult(PageResult<BlogInfo> pageResult) {
        ...
        for (int i = 0; i < blogInfos.size(); i++) {
             ...
             *
             */
            Object view = redisService.getValueByHashKey(redisKey, blogId);

            // 重新 构建缓存,删除数据Redis脏数据
            this.checkRedisHashCache(redisKey, view);
            view = redisService.getValueByHashKey(redisKey, blogId);

            blogInfo.setViews((int) view);
     ...
     * @param obj     obj
     */
    private void checkRedisHashCache(String hashKey, Object obj) {

        Optional.ofNullable(obj).orElseGet(() -> {
            // 重新 构建缓存,删除数据Redis脏数据
            Long aLong = redisService.delHashBigKey(hashKey);
            this.saveBlogViewsToRedis();
            return aLong;
        });
    }
    ...
    @Transactional(rollbackFor = Exception.class)
    @Override
    public void saveBlog(top.aqlog.model.dto.Blog blog) {
    ...
    public Blog getBlogById(Long id) {
        ...
        // 重新 构建缓存,删除数据Redis脏数据
        this.checkRedisHashCache(RedisKeyConstants.BLOG_VIEWS_MAP, view);
        view = redisService.getValueByHashKey(RedisKeyConstants.BLOG_VIEWS_MAP, blog.getId());
        blog.setViews((int) view);
        return blog;
    }
    ...
    @Transactional(rollbackFor = Exception.class)
    @Override
    public void updateBlog(top.aqlog.model.dto.Blog blog) {
